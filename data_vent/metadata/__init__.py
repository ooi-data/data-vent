...
def _get_zarr_params(table_name, params, bucket=DATA_BUCKET):
    ...
    for k, arr in zg.arrays():
        ...
        param_dict = {
            ...
            "parameter_name": arr_attrs.get("long_name", k.replace("_", " ").title()),
            "netcdf_name": k,
            "standard_name": arr_attrs.get("standard_name", None),
            "description": arr_attrs.get("comment", None),
            "unit": arr_attrs.get("units", None),
            "data_level": data_level,
            "data_product_type": arr_attrs.get("data_product_identifier", "Science Data"),
            "data_product_identifier": arr_attrs.get("data_product_identifier", None),
    ...
    if instrument_catalog_refresh:
        ...
        for _, inst in instruments_df.iterrows():
            ...
            inst_streams = streams_df[
                streams_df.reference_designator.str.match(inst_dict["reference_designator"])
            ]
            param_list = []
            for _, row in inst_streams.iterrows():
                int_pids = np.array(row["parameter_ids"].split(",")).astype(int)
                params = list(
                    map(
            ...
            inst_dict["streams"] = json.loads(inst_streams.to_json(orient="records"))
            inst_dict["parameters"] = inst_params
            instrument_catalog_list.append(inst_dict)
        json2bucket(instrument_catalog_list, "instruments_catalog.json", bucket)
...
def create_data_catalog(
    ...
    data_list = list(
        filter(
            lambda d: os.path.basename(d) not in ["index.html", "data_availability"],
    ...
    sources = map_concurrency(create_catalog_source, data_list, func_args=(FS,))
    ...
    file_contents = [
        c for c in repo.get_contents(".", ref=site_branch) if c.path == catalog_file_name
    ]
