...
def create_stream_dct(data_stream):
    try:
        ...
        data_products = [v["name"] for v in variables if v["product_id"] is not None]
...
def create_stats(s3_bucket: str = DATA_BUCKET):
    resp = requests.get("https://api.ooica.net/metadata/instruments")
    instruments = resp.json()
    instrument_refs = [i["reference_designator"] for i in instruments]
    all_files = [i for i in FS.listdir(s3_bucket, detail=True) if i["type"] == "directory"]
    valid_data_stream = []
    ...
    instrument_groups = {
        k: list(v) for k, v in groupby(final_streams, key=lambda s: s["instrument_rd"])
    }
    all_instruments = []
    for k, v in instrument_groups.items():
        logger.info(f"Indexing {k}")
        stream_total = sum(map(lambda s: s["bytes_size"], v))
        inst_dct = next(filter(lambda i: i["reference_designator"] == k, instruments))
